...
export const RecommendationDashboard: React.FC<RecommendationDashboardProps> = ({
  ...
  const filteredRecs = useMemo(() => {
    return recommendations.filter(rec => {
      ...
      // Eligibility
      if (onlyEligible && (!rec.gpa_eligible || !rec.branch_eligible)) {
        return false;
      }
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-16">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Ranked opportunities generated by TF-IDF text similarity and Random Forest fit classification.
          </p>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between text-xs text-slate-500 px-1">
          ...
          <span>Ranked by Random Forest fit & TF-IDF similarity</span>
        </div>
        ...
        {filteredRecs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredRecs.map((rec) => {
              const { job } = rec;
              const isEligible = rec.gpa_eligible && rec.branch_eligible;
                    ...
                    <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-3 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-100 min-w-[150px]">
                      ...
                      <div className="text-left sm:text-right">
                        ...
                        <span className="text-[10px] font-semibold text-slate-500">
                          Cosine: {rec.cosine_similarity_score}% • RF: {rec.rf_fit_score}%
                        </span>
                      </div>
